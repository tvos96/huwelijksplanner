...
### Overzetten vanaf de oude gedeelde-code-versie
Kies bij het inloggen "Ik gebruikte de app al met een gedeelde code" en vul de
oude code in (bijv. `tim-ita-2027`). Gasten, locaties, budget, taken, contacten
én foto's worden 1-op-1 gekopieerd naar het nieuwe, aan je Google-account
gekoppelde project. Verwijder daarna in de Firebase-console het oude document
onder `planners/{code}` (en de foto's eronder) — dat sluit ook het laatste
gaatje van de oude "iedereen met de code kan erbij"-opzet.
...
- `src/components/WeddingSetup.jsx` — nieuw project / overzetten / aansluiten
...
- `src/lib/weddingAuth.js` — inloggen, project aanmaken/overzetten/uitnodigen
